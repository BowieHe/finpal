...
- **🏢 Agent Teams 架构** — 仿照"微型投资公司"的组织架构，由 CIO（首席投资官）Agent 统一调度多个专业化 Sub-Agent。
- **⚔️ 多轮对抗辩论** — 多头分析师 vs 空头分析师进行多轮辩论（最多 3 轮），每轮由裁判 Agent 即时裁决是否继续。
- **📊 实时流式 UI** — 基于 SSE 的时间线界面，实时展示 Agent 协作进度、辩论内容、每轮裁决结果。
- **🔍 双数据源融合** — PostgreSQL 持仓数据 + 阿里云百炼 MCP 搜索实时网络情报。
- **🛑 任务中断** — 支持中途中断分析任务，通过 `AbortController` 实现服务端真正停止。
